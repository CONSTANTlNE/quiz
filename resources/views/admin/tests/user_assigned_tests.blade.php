...
@section('all.users')
    <div class="flex h-10 items-center justify-center mb-5">
        <div class="text-lg font-medium group-[.mode--light]:text-white ">
            TEST - {{$test_assignments->first()->test->test_name}}
        </div>
    </div>
    <div class="flex h-10 items-center justify-center mb-5">
        <div class="text-lg font-medium group-[.mode--light]:text-white ">
{{--            Test total Score - {{$test_assignments->first()->test->questions->sum('score')}}--}}
            ...
            <table class="w-full text-left border-b border-slate-200/60">
                ...
                @foreach($test_assignments as $index=>  $assignment)
                    <tr class="[&amp;_td]:last:border-b-0">
                        ...
                        <td class="px-5 border-b dark:border-darkmode-300 w-80 border-dashed py-4 dark:bg-darkmode-600 text-center">
                      {{-- {{$assignment->user->answers->where('test_id', $assignment->test_id)->where('correct',1)-> sum('score')}} --}}
                            {{$assignment->user_score}}
                        </td>

                        <td class="px-5 border-b dark:border-darkmode-300 w-80 border-dashed py-4 dark:bg-darkmode-600 text-center">
                            {{$assignment->user_score / $assignment->total_score * 100}}
                        </td>
